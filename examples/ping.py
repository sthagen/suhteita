...
def main(argv: Union[List[str], None] = None) -> int:
    ...
    suhteita.log.info('# Prelude of a 2-steps ping test execution')

    ts = dti.datetime.now(tz=dti.timezone.utc).strftime(suhteita.TS_FORMAT_PAYLOADS)
    suhteita.log.info(f'- Setup <01> Timestamp marker in summaries will be ({ts})')

    node_indicator = suhteita.NODE_INDICATOR
    suhteita.log.info(f'- Setup <02> Node indicator ({node_indicator})')
    suhteita.log.info(
    ...
    suhteita.log.info(f'# Starting 2-steps ping test execution at at ({start_ts})')
    ...
    suhteita.log.info(f'# Ended execution of 2-steps ping test at ({end_ts})')
    suhteita.log.info(f'Execution of 2-steps ping test took {(end_time - start_time)} h:mm:ss.uuuuuu')
    suhteita.log.info('-' * 84)
