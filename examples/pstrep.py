...
with open('ping-profiles.json', 'wt', encoding=ENCODING) as handle:
...
for transaction in scenario_labels_set:
    for node in targets:
        ...
        tmp_df = df[df['target_alias'].isin([node]) & df['scenario'].isin(['ping']) & df['step_label'].isin([transaction])]
...
for node in targets:
    print(node)
    tmp_df = df[df['target_alias'].isin([node]) & df['scenario'].isin(['ping'])]
    print(tmp_df[['total_secs', 'transactions_secs', 'duty_cycle_percent', 'step_dt_secs', 'gap_before_millis', 'gap_after_millis']].describe())
    print('-' * 42)
    print()
